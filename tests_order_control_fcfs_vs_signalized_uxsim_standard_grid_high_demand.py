# Sanity check: signalized UXsim standard vs FCFS(clearance=0) on a grid/mesh network
# under high demand (5000 and 10000 vehicles).
#
# This is NOT a research performance comparison. It checks that FCFS(clearance=0)
# does not break down under high demand and explores whether fixed 2-phase signals
# [60,60] become relatively more favorable at higher demand.
#
...
def build_world(control_mode, vehicle_plans, tmax):
    W = World(
        name=f"fcfs_vs_signalized_grid_high_demand_{control_mode}",
        deltan=1,
        tmax=tmax,
        print_mode=0,
        save_mode=0,
...
def _build_report_context(
    *,
    ...
    return (
        ...
        f"- average travel time ratio (fcfs / signalized standard): {comparison_ratios['average_travel_time_ratio']}\n"
        f"- total travel time ratio (fcfs / signalized standard): {comparison_ratios['total_travel_time_ratio']}\n"
        f"- total distance traveled ratio (fcfs / signalized standard): {comparison_ratios['total_distance_traveled_ratio']}\n\n"
        "Eligible FCFS nodes:\n"
...
def test_fcfs_clearance_zero_vs_signalized_uxsim_standard_grid_high_demand():
    for case in HIGH_DEMAND_CASES:
        _run_high_demand_case(case)

    print(
        "FCFS clearance=0 vs signalized UXsim standard grid high-demand test passed."
    )


if __name__ == "__main__":
    test_fcfs_clearance_zero_vs_signalized_uxsim_standard_grid_high_demand()
