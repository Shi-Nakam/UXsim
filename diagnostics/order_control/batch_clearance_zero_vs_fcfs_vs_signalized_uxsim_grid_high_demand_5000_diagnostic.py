...
#
# Phase 4-6N: signalized UXsim vs FCFS(clearance=0) vs Level 1 BATCH(clearance=0)
# on high-demand 6x6 grid, 5,000 vehicles only.
# Determines whether prefix violation occurs under high demand alone (clearance=0)
# or requires clearance=1 service-queue retention. Not a performance comparison.
# Records pre–node-revisit-fix state.
#
# - Not part of the normal test suite; do not add to automated regression runs.
# - BATCH exits with a known prefix violation at W.T=605 (g_4_1, veh_1619);
#   diagnostic state is printed and the exception is re-raised intentionally.
...
PREFIX_VIOLATION_PATTERN = re.compile(
    r"Batch assignment prefix violation on inlink (\S+) "
    r"at node (\S+): assigned vehicle (\S+) appears after"
)
...
def _collect_signalized_control_summary(W):
    ...
    return {
        "standard_control_type_used": (
            "UXsim standard Node.transfer with explicit 2-phase signals on "
            "internal grid nodes (order_control_type='none', "
            "set_order_control_for_nodes not called)"
        ),
        "internal_grid_signal_setting": SIGNAL_SETTING,
        "signalized_internal_grid_node_count": len(signalized_internal_nodes),
        "signalized_od_node_count": len(signalized_od_nodes),
        "signal_groups": {
            "0": "east-west links (horizontal internal links and left/right connectors)",
            "1": "north-south links (vertical internal links and top/bottom connectors)",
        },
        "signal_offsets": signal_offsets if signal_offsets else "default (0)",
        "note": (
            "UXsim standard signal control with signal=[60,60] on internal grid nodes "
            "only; external OD nodes remain unsignalized (signal=[0])"
        ),
    }
...
def _vehicle_assignment_label(veh, node_name):
    assignments = veh.order_control_batch_assignments
    if node_name in assignments:
        return f"assigned(batch_id={assignments[node_name]})"
    return "unassigned"


def _print_prefix_violation_diagnostics(W, exc):
    message = str(exc)
    print("\nBATCH prefix violation diagnostic report:")
    print(f"- exception type: {type(exc).__name__}")
    print(f"- exception message: {message}")
    print(f"- W.T: {W.T}")

    match = PREFIX_VIOLATION_PATTERN.search(message)
    if not match:
        print("- parsed node/inlink/vehicle: not available (message format unmatched)")
        return

    inlink_name, node_name, problem_vehicle_name = match.groups()
    print(f"- node name: {node_name}")
    print(f"- inlink name: {inlink_name}")
    print(f"- problem vehicle name: {problem_vehicle_name}")

    node = W.get_node(node_name)
    inlink = W.get_link(inlink_name)

    print(f"- inlink.vehicles count: {len(inlink.vehicles)}")
    print("- inlink.vehicles (front to back):")
    problem_index = None
    for index, veh in enumerate(inlink.vehicles):
        if veh.name == problem_vehicle_name:
            problem_index = index
        print(
            f"  [{index}] {veh.name}: {_vehicle_assignment_label(veh, node_name)}, "
            f"state={veh.state}, x={veh.x:.1f}, link={veh.link.name}"
        )

    if problem_index is not None:
        if problem_index > 0:
            prev_veh = inlink.vehicles[problem_index - 1]
            print(
                f"- vehicle before problem vehicle: {prev_veh.name} "
                f"({_vehicle_assignment_label(prev_veh, node_name)})"
            )
        else:
            print("- vehicle before problem vehicle: none (problem vehicle is FIFO head)")
        if problem_index + 1 < len(inlink.vehicles):
            next_veh = inlink.vehicles[problem_index + 1]
            print(
                f"- vehicle after problem vehicle: {next_veh.name} "
                f"({_vehicle_assignment_label(next_veh, node_name)})"
            )
        else:
            print("- vehicle after problem vehicle: none")

    print(f"- formal service queue length at {node_name}: "
          f"{len(node.order_control_batch_service_queue)}")
    for unit_index, service_unit in enumerate(node.order_control_batch_service_queue):
        vehicle_names = [veh.name for veh in service_unit["vehicles"]]
        print(
            f"  service_unit[{unit_index}]: batch_id={service_unit['batch_id']}, "
            f"inlink={service_unit['inlink'].name}, vehicles={vehicle_names}"
        )

    problem_veh = W.VEHICLES.get(problem_vehicle_name)
    if problem_veh is not None:
        print(
            f"- problem vehicle in incoming_vehicles at {node_name}: "
            f"{problem_veh in node.incoming_vehicles}"
        )
        print(
            f"- problem vehicle batch assignment at {node_name}: "
            f"{problem_veh.order_control_batch_assignments.get(node_name, 'unassigned')}"
        )


def build_world(control_mode, vehicle_plans, tmax):
    W = World(
        name=f"batch_fcfs_signalized_high_demand_5000_{control_mode}",
        deltan=1,
        tmax=tmax,
        print_mode=0,
        save_mode=0,
    ...
    try:
        ...
        if control_mode == "batch" and "Batch assignment prefix violation" in str(exc):
            _print_prefix_violation_diagnostics(W, exc)
        raise
...
def _format_signalized_control_summary(summary):
    return (
        "Signalized control summary:\n"
        ...
        f"- signal offsets: {summary['signal_offsets']}\n"
        f"- note: {summary['note']}"
    )
...
def _format_difference_block(title, differences):
    ...
    return (
        ...
        f"{fmt(differences['total_travel_time_difference'], 's')}\n"
        f"- average travel time difference (batch - fcfs): "
        f"{fmt(differences['average_travel_time_difference'], 's')}\n"
        f"- average delay difference (batch - fcfs): "
        f"{fmt(differences['average_delay_difference'], 's')}\n"
        f"- total distance traveled difference (batch - fcfs): "
        f"{fmt(differences['total_distance_traveled_difference'], 'm')}\n"
        ...
        f"{fmt(differences['last_completed_trip_time_difference'], 's')}\n"
        f"- max completed travel time difference (batch - fcfs): "
        f"{fmt(differences['max_completed_travel_time_difference'], 's')}"
    )
...
def _build_report_context(
    *,
    ...
    return (
        ...
        f"{_format_signalized_control_summary(signalized_control_summary)}\n\n"
        f"{_format_grid_network_summary(grid_network_summary)}\n\n"
        f"{_format_demand_summary(demand_summary)}\n\n"
        f"{_format_results('Signalized UXsim standard results', signalized_results)}\n\n"
        ...
        f"{_format_unfinished_summary('Signalized UXsim standard unfinished vehicle summary', signalized_unfinished)}\n\n"
        f"{_format_unfinished_summary('FCFS clearance=0 unfinished vehicle summary', fcfs_unfinished)}\n\n"
        f"{_format_unfinished_summary('BATCH Level 1, clearance=0 unfinished vehicle summary', batch_unfinished)}\n\n"
        f"{_format_completion_time_summary('Signalized UXsim standard completion time summary', signalized_completion)}\n\n"
        f"{_format_completion_time_summary('FCFS clearance=0 completion time summary', fcfs_completion)}\n\n"
        f"{_format_completion_time_summary('BATCH Level 1, clearance=0 completion time summary', batch_completion)}\n\n"
        f"{_format_ratio_block('Comparison ratios (FCFS / signalized UXsim)', fcfs_vs_signalized_ratios)}\n\n"
        f"{_format_ratio_block('Comparison ratios (BATCH / signalized UXsim)', batch_vs_signalized_ratios)}\n\n"
        ...
        f"Order-control clearance_timesteps: {CLEARANCE_TIMESTEPS}\n\n"
        "Sanity checks:\n"
...
def test_batch_level1_clearance_zero_vs_fcfs_vs_signalized_uxsim_grid_high_demand_5000():
    ...
    signalized_W, _ = build_world("signalized_standard", vehicle_plans, TMAX)
    signalized_control_summary = _collect_signalized_control_summary(signalized_W)
    ...
    fcfs_W, fcfs_eligible = build_world("fcfs", vehicle_plans, TMAX)
    ...
    batch_W, batch_eligible = build_world("batch", vehicle_plans, TMAX)
    ...
    sanity_checks["fcfs clearance_timesteps == 0"] = (
        "pass"
        if fcfs_W.order_control_clearance_timesteps == CLEARANCE_TIMESTEPS
        else "fail"
    )
    sanity_checks["batch clearance_timesteps == 0"] = (
        "pass"
        if batch_W.order_control_clearance_timesteps == CLEARANCE_TIMESTEPS
        else "fail"
    )
    sanity_checks["signalized standard completed ratio >= threshold"] = (
        "pass"
    ...
    assert signalized_results["completed_ratio"] >= COMPLETED_RATIO_MIN, (
        f"signalized standard completed ratio too low\n{report}"
    )
    ...
    assert (
        ...
        >= FCFS_COMPLETED_RATIO_FACTOR * signalized_results["completed_ratio"]
    ), f"fcfs completed ratio extremely worse than signalized standard\n{report}"
    assert (
        batch_results["completed_ratio"]
        >= FCFS_COMPLETED_RATIO_FACTOR * signalized_results["completed_ratio"]
    ), f"batch completed ratio extremely worse than signalized standard\n{report}"
    ...
    assert fcfs_W.order_control_clearance_timesteps == CLEARANCE_TIMESTEPS, (
        f"fcfs clearance_timesteps is not 0\n{report}"
    )
    assert batch_W.order_control_clearance_timesteps == CLEARANCE_TIMESTEPS, (
        f"batch clearance_timesteps is not 0\n{report}"
    )

    print(
        "BATCH Level 1 vs FCFS clearance=0 vs signalized UXsim grid high-demand 5000 test passed."
    )


if __name__ == "__main__":
    test_batch_level1_clearance_zero_vs_fcfs_vs_signalized_uxsim_grid_high_demand_5000()
