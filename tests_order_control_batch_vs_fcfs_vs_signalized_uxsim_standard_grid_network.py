# Grid-network comparison: signalized UXsim standard vs FCFS(clearance=0) vs
# Level 1 BATCH(clearance=0, batch_size=10) on a 6x6 grid.
#
# Extends the existing signalized UXsim vs FCFS grid-network sanity check by
# adding BATCH as a third control mode on the same network, demand, and seeds.
# Signalized UXsim uses fixed 2-phase signals on internal grid nodes; FCFS and
# BATCH use order-control without fixed UXsim signals. Reports traffic results
# for all three modes and compares BATCH with FCFS, especially average travel
# time, total travel time, and average delay. Performance ordering is NOT an
# assertion pass/fail condition; results are left for human review.
#
...
#   python tests_order_control_batch_vs_fcfs_vs_signalized_uxsim_standard_grid_network.py
#
...
def build_world(control_mode, vehicle_plans):
    W = World(
        name=f"batch_fcfs_signalized_grid_{control_mode}",
        deltan=1,
        tmax=TMAX,
        print_mode=0,
        save_mode=0,
...
def _format_difference_block(title, differences):
    return (
        ...
        f"{differences['total_distance_traveled_difference']:.1f} m"
    )
...
def test_batch_level1_clearance_zero_vs_fcfs_vs_signalized_uxsim_grid_network():
    vehicle_plans = _generate_vehicle_plans()
    demand_summary = _demand_summary(vehicle_plans)

    signalized_W, _ = build_world("signalized_standard", vehicle_plans)
    ...
    sanity_checks["signalized internal grid node count matches existing"] = (
        "pass"
        if signalized_control_summary["signalized_internal_grid_node_count"]
        == INTERNAL_GRID_NODE_COUNT
        else "fail"
    )
    sanity_checks["external OD nodes remain unsignalized"] = (
        "pass"
    ...
    sanity_checks["signalized UXsim completed ratio >= threshold"] = (
        "pass"
    ...
    assert signalized_results["completed_ratio"] >= COMPLETED_RATIO_MIN, (
        f"signalized UXsim completed ratio too low\n{report}"
    )
    ...
    assert (
        ...
        >= FCFS_COMPLETED_RATIO_FACTOR * signalized_results["completed_ratio"]
    ), f"fcfs completed ratio extremely worse than signalized UXsim\n{report}"
    assert (
        batch_results["completed_ratio"]
        >= FCFS_COMPLETED_RATIO_FACTOR * signalized_results["completed_ratio"]
    ), f"batch completed ratio extremely worse than signalized UXsim\n{report}"
    ...
    print(
        "BATCH Level 1 vs FCFS clearance=0 vs signalized UXsim grid network test passed."
    )


if __name__ == "__main__":
    test_batch_level1_clearance_zero_vs_fcfs_vs_signalized_uxsim_grid_network()
