...
進捗の詳細・コミットID・回帰結果は [ORDER_EXCHANGE_PROGRESS.md](ORDER_EXCHANGE_PROGRESS.md) を参照。Phase 4-6E〜4-6Jの設計・判断経緯の詳細は **§1C**、Phase 4-6Kの実装記録は **§1D**、Phase 4-6Lの統括メソッド記録は **§1E**、Phase 4-6Mの `Node.transfer()` 接続記録は **§1F**、Phase 4-6Nの比較・診断記録は **§1G**、Node訪問単位の共通状態設計は **§1H**（zero-service追加形成・size-one BATCHとFCFS等価性は **§1H.25**、模倣World型Level 2 t_trigger参照モデルは **§1H.26**、**フェーズ4-6Y**（Level 2本体接続・実ネットワーク診断・BATCH残作業整理とTime-value Transactionへの移行判断）は **§1H.27.42**、**§1H.27.43**、**§1H.27.44**、**§1H.27.45**、**§1H.27.46**）を参照。
...
| **現時点の主要課題** | Time-value Transactionの設計・実装（Phase名・実装範囲は未決定）。BATCH残作業は§1H.27.46へ整理済み。複数seed、別network、Vehicle別・Node別分析、統計的検定はTime-value Transaction実装後の共通評価段階へ繰り越す。BATCH固有のN・horizon感度分析、Level 2追加性能改善は保留する |
...
| **次工程候補** | **§1H.27.46** を参照。**次の本体対象：** Time-value Transactionの設計・実装（Phase名、詳細区分、実装範囲は未決定）。**BATCH：** BATCH単独の追加探索はいったん停止。batch_size=10、通常Level 2、unresolved時Level 1 fallback、virtual horizon 30を暫定ベースラインとして維持（正式値・最適値ではない）。**共通評価へ繰り越し：** 複数seed、別network、Vehicle別・Node別分析、統計的検定。**BATCH固有課題として保留：** N感度、horizon 30・50以外の体系的探索、Level 2追加性能改善。**必要性確認後：** Level 0自動fallback、stale service unit、assignment全訪問履歴。**研究対象外：** trip-end Vehicle。**完了済み（フェーズ4-6Y）：** Level 2本体接続（§1H.27.42）、5,000台・10,000台Level 1対Level 2比較（§1H.27.43・§1H.27.45）、200/1,000/5,000/10,000台N=1 BATCH Level 2対FCFS一致性（§1H.27.44・§1H.27.45）、mimic World Analyzer省略（§1H.27.44）、指定条件でのvirtual horizon 30対50限定比較（§1H.27.45）、5,000台補正signalized UXsim比較（§1H.27.45） |
...
| **4-6W** | 模倣World型Level 2 t_trigger参照モデル・専用テスト（**§1H.26**。参照モデル・独立レビュー完了。本体未接続。`uxsim/uxsim.py` 未変更） |
...
### 1H.26 Phase 4-6W：模倣World型Level 2 t_trigger参照モデル

#### 1H.26.1 位置付けと目的

Phase 4-6Wは、Level 2 t_trigger estimatorの**本体実装ではない**。本体接続前に用いる、**模倣World型Level 2 t_trigger最小参照モデル**を確立した。
...
**未実施（現時点）：** t_trigger Level 2の本体有効化、`form_order_control_batch()` への接続、10,000台simulation、200台diagnostic、network-wide性能評価、組合せ探索、local virtual clock実装、topology再利用、大規模性能最適化。

**実装ファイル（diagnostics配下・本体未接続）：**

- `diagnostics/order_control/level2_virtual_world_reference.py`
...
参照モデルは実Worldを変更せず、呼び出しごとに小規模mimic Worldを構築する。Level 0・Level 1 estimatorは未変更。

#### 1H.26.2 模倣World型参照モデルを先行作成した理由

Level 2本体を直接有効化する前に、次を小規模で固定する必要があった。

- BATCH serve規則・capacity・clearance・outlink空間の相互作用
- trigger単独の条件付き仮想サービス時刻の意味
- real snapshotからmimic Worldへの状態写像の完全性

模倣Worldを毎回構築する方式は、local virtual clock実装や本体直接接続と比較するための基準実装として位置付ける。

#### 1H.26.3 Phase名称管理上の失敗と再発防止規則

通常工程ではMarkdown上のPhase記号とコミット件名の `phase 4-6X:` 形式を対応させていた。テストエラー修正期間ではコミット件名にPhase記号を使わない方針とした一方、Markdown内部ではPhase 4-6U・4-6Vを消費し、Gitコミット件名とMarkdown内部Phase連番の対応が失われた。これは工程名称管理上の失敗である。既存のpush済みコミット名・過去MarkdownのPhase名は変更しない。
...
#### 1H.26.4 mimic World構造

```
dummy upstream Nodes
    ↓
mimic inlinks
    ↓
mimic order-control Node
    ↓
mimic outlinks
    ↓
sink Nodes
...
- **dummy upstream Node：** mimic inlinkの構造上端点。Vehicle生成なし・新規流入なし・signalなし・order-controlなし
- **sink Node：** mimic outlink終端。outlink上Vehicleのdestination。UXsim標準end-trip処理を使用

#### 1H.26.5 real→mimic写像
...
| Node | mimic Node |
| inlink | mimic inlink |
| outlink | mimic outlink |
| Vehicle | mimic Vehicle |

**inlink写像により再構築：** service unit所属inlink、`Vehicle.link`、`last_order_control_inlink`、`capacity_out`参照先、各inlinkのFIFO順、同一inlink継続またはinlink変更、clearance要否。

**outlink写像により再構築：** `Vehicle.route_next_link`、`capacity_in`参照先、outlink空間判定、対象Node通過後の流入先。

**clearance状態写像：**

- `real_node.last_order_control_inlink` → 対応mimic inlink → `mimic_node.last_order_control_inlink`
- `real_node.last_order_control_entry_timestep` → 同じ整数値 → `mimic_node.last_order_control_entry_timestep`
- `real_node.order_control_clearance_timesteps` → 同じ整数値 → `mimic_node.order_control_clearance_timesteps`

mimic Worldはreal Worldのsnapshot W.Tから開始するため、entry timestepは相対値へ変換しない。

#### 1H.26.6 service queueの模倣

real service queueはdeepcopyせず、mimic参照で再構築する。

各mimic service unitが保持するもの：batch_id、mimic inlink、mimic Vehicle list、visit_ids、queue順序、unit内Vehicle順序。

既存service unitのbatch_idはmimic Worldでも同じ値。各mimic Vehicleのbatch assignmentをunitのbatch_idと一致させる。

**trigger疑似unit：**

- mimic trigger Vehicle 1台、triggerのmimic inlink、triggerのmimic visit_id
- 既存mimic service unitで未使用のbatch_id
- mimic service queue末尾へ追加

triggerはreal Worldではbatch化しない。real Worldではassignment・current visit・service queue・batch ID counterを変更しない。

#### 1H.26.7 triggerの扱い

triggerは現行trigger rank key（`arrival_time`・`arrival_tiebreaker`・Vehicle ID）で**選択済みの1台**を引数で受け取る。参照モデルは選び直さない。

trigger後方の未assignment Vehicleはmimic Worldへ含めない。理由：Level 2はtrigger自身の仮想通過時刻を求める。正式batch memberはLevel 2値取得後に既存形成経路で決める。trigger後方Vehicleはtrigger自身の通過を早めない。

#### 1H.26.8 route_next_link固定

mimic Vehicleの`route_next_link`はtrigger選択時点のsnapshot値へ固定する。仮想計算中は`route_next_link_choice()`・route search・route preference更新・DUO更新を行わない。

**参照モデルの意味：** trigger選択時点のroute_next_linkを固定した条件付き仮想サービス時刻。

実networkでは待機中にroute_next_linkが再選択され得る。これは参照モデルとの差異である。mimic World固有の交通状況に基づく再選択を混ぜるより、snapshot固定の方が予測条件を明確にできるため、初期参照モデルでは固定している。

#### 1H.26.9 capacity snapshotと補充の境界

snapshot時点のcapacity残量を対応mimic属性へコピーする。

- `real_inlink.capacity_out_remain` → `mimic_inlink.capacity_out_remain`
- `real_outlink.capacity_in_remain` → `mimic_outlink.capacity_in_remain`
- `real_node.flow_capacity_remain` → `mimic_node.flow_capacity_remain`

snapshot W.Tでは、当該timestepのLink.update()・Node.update()による補充が既に反映済み。よってmimic計算の最初の同じW.Tでは再補充しない。

**仮想時間の数え方：**

| offset | 意味 |
|--------|------|
| 0 | snapshotと同じW.T。capacity補充なし。snapshot残量でserve |
| ≥1 | 新しいW.Tへ進む。UXsim既存式でLink capacity・Node flow capacityを補充してからserve |

仮想通過時には既存serve処理によりcapacityが消費される。

#### 1H.26.10 同一timestep内の複数台通過

同一inlinkの複数Vehicleについて、Vehicleごとに1 timestepを強制しない。capacity・outlink空間・到着条件が満たされる場合、既存BATCH serve処理により同じW.Tで複数台通過できる。

専用テストで確認：A1通過T=10、A2通過T=10、trigger通過T=11。

#### 1H.26.11 outlink Vehicle移動とsink end-trip

関連outlink上の全Vehicleをmimic Worldへ複製し、UXsim car-followingで前進させる。

**入口空間回復テスト：** T=10でblockerがoutlink入口x=0にありtriggerは流入不能。T=11でblocker前進後に入口空間回復しtriggerが流入。この時点ではblockerのsink end-tripは未発生。

**sink end-tripテスト：** blockerをoutlink終端近くから開始。T=10にsink到達・`flag_waiting_for_trip_end`設定・`end_trip()`実行。`end_trip()`後にoutlink.vehiclesから除去されたことを確認してから`outlink_removal_timestep`を記録。入口空間回復テストとは分離して確認。

#### 1H.26.12 t_virtual_triggerと暫定候補式

**t_virtual_trigger：** trigger Vehicleがmimic order-control Nodeからmimic outlinkへ仮想transferされた絶対timestep。

**参照モデルで確認した暫定候補式（本体未接続）：**

- 仮想計算成功時：`t_level_2_candidate = max(t_level_1, t_virtual_trigger)`
- `t_level_1`：現行Level 1 estimatorのint timestep
...
正式なLevel 2式として最終確定したものではない。

#### 1H.26.13 virtual horizon

virtual horizonは、snapshotからtriggerが通過しないまま進めてよい仮想timestep数の上限。

horizon内にtriggerが通過しない場合：`resolved=False`、`t_virtual_trigger=None`、`t_level_2_candidate=t_level_1`、`reason="virtual_horizon_exceeded"`。これは正常な非適用でありValueErrorではない。研究上の正式値は未決定。

#### 1H.26.14 正常非適用と重大不整合の区別

**正常非適用：** virtual horizon超過など。`resolved=False`と`reason`で返す。

**重大不整合（ValueError）：** 少なくとも次を検出する。

- clearance履歴の片側欠落
- service unit必須キー欠落
- vehiclesとvisit_idsの長さ不一致
- 空service unit
- Vehicleの複数service unit重複
- service unitのinlinkとVehicle.linkの不一致
- service unitのvisit_idとcurrent visitの不一致
- service unitのbatch_idとVehicle assignmentの不一致
- assignment済みVehicleがinlink内で連続prefixを形成しない
- assignment済みprefix Vehicleがservice queueに存在しない
- triggerより前方のVehicleが未assignment
- triggerより前方のassignment済みVehicleがservice queueに存在しない
...
- `vehicle_transfer_timesteps`：各Vehicleがmimic order-control Nodeを通過したW.T
- `sink_end_trip_trace`：sink関連Vehicleについて `sink_arrival_timestep`、`flag_waiting_for_trip_end_timestep`、`end_trip_timestep`、`outlink_removal_timestep`（`end_trip()`実行後の実除去確認後に記録）

#### 1H.26.16 専用テスト結果

**18/18 PASS**（`tests_order_control_batch_t_trigger_level_2_reference.py`）
...
1. Phase 4-6W参照モデルを基準に、適用範囲と性能測定計画を確定
2. 本体用local virtual clockへ移植するか、模倣Worldを直接使用するかを比較
3. 本体Level 2接続仕様を確定
...
### 1H.27 Phase 4-6X：未到着service-unit Vehicle対応と参照モデル専用service処理の実装前確定設計

#### 1H.27.1 位置付け

Phase 4-6Xは、Phase 4-6W模倣World型Level 2 t_trigger参照モデルを拡張するための**実装前確定設計**である。

**今回の位置付け：**

- 未到着service-unit Vehicle対応および参照モデル専用service処理の**実装前設計を確定した**
- Python実装・専用テスト・performance benchmark・UXsim本体Level 2接続・t_trigger level 2有効化・virtual horizon正式値の決定は**未着手**

**避ける表現：** Phase 4-6X実装済み、未到着Vehicle対応済み、Level 2完成、最適outlinkを選ぶ、負荷を均等化する、実際のroute_next_linkを予測する、traffic performanceを改善する。

**優先する表現：** 実装前確定設計、楽観的な仮想流入先、決定論的な分散、acceptable_outlinksへの剰余選択、参照モデル専用service処理、本体未接続、実装・テスト未着手、最適負荷分散は保証しない。

Phase 4-6XでもLevel 2はUXsim本体へ接続しない。`uxsim/uxsim.py`は変更しない。

#### 1H.27.2 問題の発見経緯（Phase 4-6W制約）

Phase 4-6W参照モデルは、snapshot時点でNode端へ到着済みのservice-unit Vehicleを中心に検証した。

一方、現行BATCH形成では次が成立する。

- candidateは`inlink.vehicles`から抽出される
- candidate条件に`incoming_vehicles`所属は含まれない
- `earliest_arrival_timestep <= t_trigger`であれば、Node端未到着Vehicleもservice unitへ登録され得る

Phase 4-6W参照モデルでは、inlink上のVehicleに通常の`Vehicle.update()`を呼ばず、inlink Vehicleを前進させていない。

そのため、service unitの先頭Vehicleがsnapshot時点で未到着の場合、次が起こり得る。

- Vehicleの位置が進まない
- `incoming_vehicles`へ登録されない
- arrival waitが解消しない
- 実networkでは後に通過できる状態でも、参照モデルではvirtual horizon超過となり得る

Phase 4-6W参照モデルは到着済みVehicleの小規模基準モデルとしては有効である。参照モデルによって**次の未対応範囲を具体的に特定できた**と位置付ける。Phase 4-6Wの価値を否定するものではない。

#### 1H.27.3 Phase 4-6Xの目的

1. 既存service queueに登録済みでNode端へ未到着のVehicleを、mimic World内で仮想前進させる
2. VehicleをNode端へ仮想到着させる
3. snapshot時点で`route_next_link`が確定済みのVehicleと未確定のVehicleを区別する
4. 出口未確定Vehicleについて、実networkの将来routeを予測せず、仮想transfer時点で楽観的な仮想outlinkを選ぶ
5. Vehicleを1台ずつ最新状態で評価する、参照モデル専用service処理を設計する
6. 同一virtual timestep内で複数Vehicleを逐次transferできるようにする
7. 通過不能理由ごとに、queue全体停止、inlink単位停止、通過後停止、重大不整合を区別する

**今回の目的ではないもの：** 本体Level 2の接続・有効化、network-wide simulation、performance benchmark、local virtual clock、組合せ探索、最適なoutlink負荷分散。

#### 1H.27.4 研究対象外条件

BATCHプロセスでは次を**一切扱わない**（Phase 4-6Xだけの初期スコープ外ではなく、研究対象外の前提条件。将来対応事項としても記載しない）。
...
- 単車線
- BATCH対象NodeはVehicleのdestinationではない
- snapshot時点で出口未確定のVehicleについて、対象Nodeの全物理outlinkを楽観的な仮想流入先候補にできる

#### 1H.27.5 用語の正式定義

**参照モデル専用service処理：** mimic World内でservice queueを処理し、Vehicleをinlinkからoutlinkへ仮想transferする処理。通常の`Node.transfer()`全体ではない。Vehicle生成、通常のroute choice、signal処理等は行わない。

**Type A Vehicle：** snapshot時点で、対象Node通過後のoutlinkが`route_next_link`として確定済みのVehicle。条件：`route_next_link`が対象Nodeの`outlinks`に含まれる。Type Aはsnapshotの`route_next_link`を固定し、別outlinkへ変更しない。

**Type B Vehicle：** snapshot時点で、対象Node通過後のoutlinkが未確定のVehicle。条件：`route_next_link is None`、または`route_next_link is veh.link`。後者は、現在のinlinkへ進入する際に使用されたLink参照が残っており、対象Node通過後のoutlinkがまだ選択されていない状態を意味する。

**acceptable_outlinks：** 現在評価中のVehicleについて、現在のvirtual W.Tで次の条件を**両方**満たす、対象Nodeの物理outlinkの集合。

- `outlink.capacity_in_remain >= DELTAN`
- outlink入口に必要な空間がある

各Vehicleの評価時に、その時点の最新状態から作り直す。

**blocked_inlinks：** 同一virtual timestepのservice処理中に、BLOCK_INLINKと判定されたinlinkの集合。service処理開始時は空集合。BLOCK_INLINK発生時に対象inlinkを追加する。

**SKIP_INLINK：** `blocked_inlinks`に含まれるinlinkのservice unitが、queue走査の後方で再び現れた場合、そのunitを評価せず飛ばす処理。新しい通過不能理由ではなく、同じinlinkを同一service処理内で再評価しないための処理。

**active_inlink：** 同一virtual timestep内で、直近にVehicleを正常transferしたinlink。service処理開始時は`None`。TRANSFER_OKごとに、transferしたVehicleのinlinkを設定する。1台以上transfer後は異なるinlinkへ切り替えないため、同一virtual timestep内では結果として最初にtransferしたinlinkと同じままとなる。

**virtual_node_arrival_timesteps：** mimic Vehicleがmimic Node端へ仮想到着したvirtual W.Tを、Vehicle名をキーとして保持する診断記録。例：`virtual_node_arrival_timesteps["A1"] = 10`。service順、outlink選択、通過可否を決める状態ではない。

#### 1H.27.6 snapshot時点

Level 2は将来、`Node.transfer()`内のBATCH形成処理から呼ばれる想定である。

**同一W.TのLevel 2呼出し時点で完了済み：**

- Link capacity補充
- Node flow capacity補充
- Node update
- 直前timestep末までのVehicle位置更新

**同一W.TのLevel 2呼出し後に行われる処理：**
...
したがって、snapshotのVehicle位置は、当該W.TのVehicle前進前の位置である。

#### 1H.27.7 virtual timestep全体の正式フロー

**10.1 offset=0（正式テキストフロー）**
...
capacityは再補充しない
    ↓
snapshot位置とsnapshot capacity残量で
参照モデル専用service処理を実行
    ↓
triggerが通過したか
    ├─ YES
    │    ↓
    │  t_virtual_triggerを記録
    │    ↓
    │  その時点で仮想計算を終了
    │
    └─ NO
         ↓
       既存service queueに登録された
       inlink Vehicleを仮想前進
         ↓
       関連outlink上のVehicleを仮想前進
         ↓
       inlink終端へ到達したVehicleを
       mimic Node.incoming_vehiclesへ登録
         ↓
       virtual_node_arrival_timestepsへ記録
         ↓
       sink処理
         ↓
       virtual horizon判定
         ↓
       horizon未到達なら次のvirtual W.Tへ（§10.2 offset>=1の処理へ）
```

**10.2 offset>=1（正式テキストフロー）**

```
mimic WorldのW.Tを1進める
    ↓
Link capacityとNode flow capacityを補充
    ↓
参照モデル専用service処理を実行
    ↓
triggerが通過したか
    ...
    └─ NO
       ...
       virtual horizon判定
         ↓
       horizon未到達なら次のvirtual W.Tへ
```

**重要点：**

- offset=0のservice処理**前**にVehicleを動かさない
- offset=0のservice処理**後**に最初のVehicle前進を行う
- offset=0でもhorizon未到達ならW.Tを1進めてoffset>=1の処理へ進む
- service後に仮想到着したVehicleは**次のvirtual W.T**からservice対象
- trigger通過後は、そのoffset内の前進、到着登録、sink、horizon判定を行わず終了

**Mermaid補助図（virtual timestep全体）：**

```mermaid
flowchart TD
    START0([offset=0: snapshot W.T])
    NOREFILL[capacity再補充しない]
    SERVE[参照モデル専用service処理]
    TRIG{trigger通過?}
    RECORD[t_virtual_trigger記録して終了]
    ADV[inlink Vehicle仮想前進]
    ADV2[関連outlink Vehicle仮想前進]
    ARRIVE[incoming_vehicles登録]
    TRACE[virtual_node_arrival_timesteps記録]
    SINK[sink処理]
    HORIZON{virtual horizonへ到達したか}
    UNRESOLVED([unresolved結果で終了])
    WTP1[mimic WorldのW.Tを1進める]
    REFILL[Link capacityとNode flow capacity補充]
    ...
    TRACE --> SINK
    SINK --> HORIZON
    HORIZON -->|YES| UNRESOLVED
    HORIZON -->|NO| WTP1
    WTP1 --> REFILL
    REFILL --> SERVE
```

#### 1H.27.8 未到着Vehicleの仮想前進

**inlink側の仮想前進対象：**

- 既存service queueに登録済み
- snapshot時点でmimic Node.`incoming_vehicles`に未登録
- mimic inlink上に存在

mimic inlink内のFIFOとleader・followerを維持する。
...
x = x_next
    ↓
必要に応じてxをinlink.lengthへ固定
```

通常の`Vehicle.update()`全体は呼ばない。理由：`route_next_link_choice()`を呼ばない。mimic World固有の状態から実networkの将来routeを予測しない。

**Node端到達時：**

- `x`を`inlink.length`へ固定
- `incoming_vehicles`へ重複なく追加
- `virtual_node_arrival_timesteps`へvirtual W.Tを記録

**仮想到着時に変更しないもの：** current visitの`arrival_time`・`arrival_tiebreaker`、`earliest_arrival_timestep`、batch assignment、service unit。

**呼ばないもの：** `record_order_control_node_arrival()`、実WorldのRNG。

#### 1H.27.9 route状態分類

各Vehicleについて、**交通上の停止条件より先に**route状態を分類する。

**Type A：** `route_next_link`が対象Nodeの`outlinks`に含まれる。

**Type B：** `route_next_link is None`、または`route_next_link is veh.link`。

**異常route：** `route_next_link`が非Noneであり、`veh.link`でも対象Nodeのoutlinkでもない。Type Bへ変換せず、**VALUE_ERROR**。

#### 1H.27.10 acceptable_outlinksの作成

各Vehicle評価時に**一度だけ**作る。対象Nodeの全物理outlinkのうち、次を両方満たすものを収集する。
...
- `outlink.vehicles`が空
- または最後尾Vehicleの`x`が必要車間距離より大きい

`acceptable_outlinks`が空：全物理outlink受入不能 → **STOP_QUEUE**。

先行Vehicleのtransfer後は状態が変わるため、次Vehicleでは**新しく作り直す**。

#### 1H.27.11 Type Bのoutlink選択（正式規則）

`acceptable_outlinks`が空でない場合のみ実行する。

```
acceptable_outlinksをoutlink.id昇順に並べる
    ↓
mimic Vehicleに対応するreal Vehicleを取得
    ↓
real_vehicle_id = real Vehicle.id
    ↓
selection_index = real_vehicle_id % len(acceptable_outlinks)
    ↓
chosen_outlink = acceptable_outlinks[selection_index]
    ↓
chosen_outlinkへ同じ処理内で直ちにtransfer
```

`mimic Vehicle.id`は変更しない。逆写像 `mimic Vehicle → real Vehicle` を使用する。

**撤回した旧方式：** 全物理outlink数に対して剰余を取り、受入不能outlinkを飛ばして循環探索する方式。

**撤回理由：** 受入不能outlinkの次に位置するoutlinkへ偏り得る。`acceptable_outlinks`への剰余選択の方が単純で、決定論的に分散しやすい。

**注意：** 最適負荷分散を保証するものではない。乱数を使わない。IDの小さいoutlinkへの固定的集中を抑える。同一入力で同一結果を得ることが目的。

#### 1H.27.12 Type Aの固定outlink判定

`acceptable_outlinks`が空でないことを確認した後、固定outlinkが集合に含まれるか確認する。

**含まれる：** 固定outlinkへtransfer → **TRANSFER_OK**。

**含まれない：** Type Aは別outlinkへ変更しない。`acceptable_outlinks`自体は空ではない（Node全体が通過不能ではない。固定outlinkだけが現在受入不能）。

- まだ同一virtual timestepで1台もtransferしていない → **BLOCK_INLINK**、当該inlinkを`blocked_inlinks`へ追加、異なるinlinkの後続unitを評価可能
- 同じ`active_inlink`から1台以上transfer済み → **STOP_AFTER_TRANSFER**、当該service処理を終了

#### 1H.27.13 Vehicle単位の正式フロー

```
service unitの先頭Vehicleを取得
    ↓
service unit・visit・assignment・inlinkの整合を検証
    ├─ 不整合 → VALUE_ERROR
    └─ 正常
         ↓
       route状態を分類
         ├─ 想定外route → VALUE_ERROR
         ├─ Type A
         └─ Type B
              ↓
       incoming_vehiclesに存在するか
         ├─ NO → STOP_QUEUE (arrival wait)
         └─ YES
              ↓
       clearanceを満たすか
         ├─ NO → STOP_QUEUE
       ...
       inlink FIFO先頭か
         ...
         └─ YES
              ↓
       Node flow capacityは十分か
         ├─ NO → STOP_QUEUE
         └─ YES
              ↓
       acceptable_outlinksを作成
         ↓
       acceptable_outlinksは空か
         ├─ YES → STOP_QUEUE
         └─ NO
              ↓
       inlink capacity-outは十分か
         ├─ NO
         │    ├─ transferred_vehicle_count == 0 → BLOCK_INLINK
         │    └─ transferred_vehicle_count >= 1 → STOP_AFTER_TRANSFER
         └─ YES
              ↓
       Type A / Type B分岐
```
...
Type A
    ↓
固定outlinkはacceptable_outlinksに含まれるか
    ├─ YES → 固定outlinkへtransfer → TRANSFER_OK
    └─ NO
         ↓
       別outlinkへは変更しない
         ↓
       transferred_vehicle_count == 0か
         ├─ YES → BLOCK_INLINK
         └─ NO → STOP_AFTER_TRANSFER
```
...
selection_index = real_vehicle_id % len(acceptable_outlinks)
    ↓
acceptable_outlinks[selection_index]へ同じ処理内で直ちにtransfer
```

Type A・Type B分岐後にcapacity-inと入口空間を**再計算しない**（`acceptable_outlinks`作成時点で確定）。

**Mermaid補助図（Vehicle単位・queue走査の統合概要）：**

```mermaid
flowchart TD
    QSTART[service queue先頭から走査]
    SKIP{inlink in blocked_inlinks<br/>かつ transferred==0?}
    SKIP -->|YES| SKIPUNIT[SKIP_INLINK]
    SKIP -->|NO| SWITCH{transferred>=1 かつ<br/>inlink != active_inlink?}
    SWITCH -->|YES| QEND[service処理終了]
    SWITCH -->|NO| VSTART[Vehicle単位フロー]
    VSTART --> ROUTE[route分類]
    ROUTE --> CHECKS[arrival/clearance/FIFO/<br/>node cap/acceptable_outlinks/<br/>inlink cap-out]
    CHECKS --> BRANCH{Type A or B}
    BRANCH -->|A| FIX[固定outlink判定]
    BRANCH -->|B| MOD[acceptable_outlinks剰余選択]
    FIX --> RESULT{結果}
    MOD --> RESULT
    RESULT -->|TRANSFER_OK| UPDATE[状態更新・transferred++]
    UPDATE --> VSTART
    RESULT -->|BLOCK_INLINK| BLK[blocked_inlinks追加]
    BLK --> QSTART
    RESULT -->|STOP_*| QEND
    SKIPUNIT --> QSTART
```

#### 1H.27.14 service queue走査の正式フロー
...
```
service queueを先頭から順に走査
    ↓
transferred_vehicle_count == 0
かつ
現在unitのinlinkがblocked_inlinksに含まれるか
    ├─ YES → SKIP_INLINK → queue上の次unitへ
    └─ NO
         ↓
       transferred_vehicle_count >= 1か
         ├─ NO → 現在unitを評価
         └─ YES
              ↓
            現在unitのinlinkはactive_inlinkと同じか
              ├─ NO → 当該virtual timestepのservice処理を終了
              └─ YES → 現在unitを評価
```
...
```
unitの先頭VehicleをVehicle単位フローで評価
    ↓
結果は何か
    ├─ TRANSFER_OK
    │    ↓
    │  transfer後の状態を即時更新
    │    ↓
    │  transferred_vehicle_countを1増やす
    │    ↓
    │  active_inlinkを今回のinlinkへ設定
    │    ↓
    │  unitに次Vehicleがあるか
    │    ├─ YES → 同じunitの次Vehicleを評価
    │    └─ NO → queue上の次unitへ
    │
    ├─ BLOCK_INLINK
    │    ↓
    │  当該inlinkをblocked_inlinksへ追加
    │    ↓
    │  queue上の次unitへ
    │
    ├─ STOP_AFTER_TRANSFER → 当該service処理を終了
    ├─ STOP_QUEUE → 当該service処理を終了
    └─ VALUE_ERROR → 例外で停止
```

**重要：**

- queue後方にある同じinlinkのunitを検索して先に処理しない
- 現在unitが空になった後、queue上で次に評価されるべきunitが同じ`active_inlink`なら継続可能
- `transferred_vehicle_count == 0`かつ`blocked_inlinks`に含まれるinlinkの後続unitはSKIP_INLINK
- `transferred_vehicle_count >= 1`で異なるinlinkのunitへ到達した時点で、当該virtual timestepのservice処理を終了する（SKIPして走査を継続しない）

#### 1H.27.15 A1・B1・A2の具体例
...
**B1でclearance充足済みだがBLOCK_INLINK：**

```
blocked_inlinks = {inlink A, inlink B}
transferred_vehicle_count = 0
    ↓
queue走査はA2まで進む
```
...
A2は同じtimestep中に新しく形成されたunitではなく、service処理開始時からqueue後方に存在していたunitである。

#### 1H.27.16 transfer後の即時更新

Vehicleのtransfer成功直後に、既存serveの意味と一致させて、少なくとも次を更新する設計である。
...
- service unitの`vehicles`
- service unitの`visit_ids`
- Link累積流入・流出台数
- `Vehicle.link`
- Vehicle位置、lane、leader、follower
- assignment完了
- current visit完了

更新後の状態で、同一virtual W.T内の次Vehicleを評価する。VehicleごとにW.Tは進めない。

#### 1H.27.17 clearance履歴

Vehicleがmimic Nodeを正常に通過した場合：

```
last_order_control_inlink = 今回Vehicleが来たmimic inlink
last_order_control_entry_timestep = 今回Vehicleが通過したvirtual W.T
```

次のservice unitが異なるinlinkの場合、この2値とclearance設定を使ってclearance充足を判定する。

#### 1H.27.18 停止理由の正式定義

**STOP_QUEUE：** arrival wait、clearance未充足、Node flow capacity不足、`acceptable_outlinks`が空。

**BLOCK_INLINK：** inlink capacity-out不足、Type Aの固定outlinkが`acceptable_outlinks`に含まれない（`transferred_vehicle_count == 0`の場合に使用）。

**STOP_AFTER_TRANSFER：** 同じ`active_inlink`から1台以上transfer済み。そのinlinkの次VehicleがSTOP_QUEUEまたはVALUE_ERROR以外の通常理由で通過不能。

**SKIP_INLINK：** `blocked_inlinks`に含まれるinlinkの後続service unitを評価せず飛ばす処理。

**VALUE_ERROR：** service unit・visit・assignment・inlink不整合、FIFO不整合、想定外の非None `route_next_link`、Phase 4-6Wと同型のその他重大不整合。

**TRANSFER_OK：** Vehicleの仮想transfer成功。

#### 1H.27.19 同一virtual timestep内の継続規則

**まだ1台もtransferしていない場合：**

| 結果 | 継続 |
|------|------|
| STOP_QUEUE | queue全体を終了 |
| BLOCK_INLINK | 当該inlinkをblockedにする。異なるinlinkの後続unitを評価 |
| SKIP_INLINK | 当該unitを飛ばして後続unitを評価 |
| VALUE_ERROR | 即時停止 |
| TRANSFER_OK | 次Vehicleまたは次service unitを評価 |

**すでに1台以上transferしている場合：**

- 同じ`active_inlink`の次Vehicleを評価できる
- 同じ`active_inlink`の次service unitも評価できる
- 異なるinlinkのservice unitは評価せず終了する
- 同じ`active_inlink`の次Vehicleが通常理由で通過不能ならSTOP_AFTER_TRANSFER
- STOP_QUEUE理由ならqueue全体を終了
- VALUE_ERRORなら即時停止

#### 1H.27.20 実装時の責務分割候補

実装前設計として、次の小さな責務に分ける候補を記録する（関数名は仮。実装段階で既存コードとの整合を確認し、必要なら変更できる）。

- `validate_service_unit_vehicle()`
- `classify_route_state()`
- `check_arrival()`
- `check_clearance()`
- `check_fifo()`
- `check_node_capacity()`
- `check_inlink_capacity()`
- `get_acceptable_outlinks()`
- `choose_outlink_by_vehicle_id()`
- `transfer_vehicle_reference()`
- `serve_reference_batch_queue()`

既存serve全体の無秩序なコピーを避け、処理順、停止理由、Type Bのoutlink選択を明示することが目的である。

#### 1H.27.21 Phase 4-6Wから維持する仕様
...
- 実World RNGを変更しない
- triggerは選択済み1台
- triggerのsnapshot `route_next_link`を固定
- trigger後方Vehicleを含めない
- triggerはmimic World内の単独疑似service unit
- capacity snapshotを継続
- offset=0ではcapacityを再補充しない
- 関連outlink上の全Vehicleを前進
- sinkで標準end-trip
- `t_level_2_candidate = max(t_level_1, t_virtual_trigger)`
- virtual horizon超過は通常非適用
- 重大不整合はValueError
- `uxsim/uxsim.py`を変更しない
- 本体Level 2へ接続しない

#### 1H.27.22 実装前のテスト計画

今回テストは作成しない。実装時に必要なテスト計画を記録する。

1. offset=0ではservice前にVehicleが動かず、service後に1回だけ前進する
2. 未到着service-unit Vehicleが仮想前進する
3. Node端到着後、`incoming_vehicles`へ登録される
4. `virtual_node_arrival_timesteps`へ正しい時刻を記録する
5. 仮想到着したVehicleは次のvirtual W.Tからservice対象になる
6. Type Aはsnapshot固定outlinkを維持する
7. Type Bは`acceptable_outlinks`からVehicle IDの剰余で決定論的に選ぶ
8. 受入可能outlinkが2本、3本の場合の剰余選択
9. `acceptable_outlinks`が空ならSTOP_QUEUE
10. Type Aの固定outlinkだけが受入不能ならBLOCK_INLINK
11. Node flow capacity不足ならSTOP_QUEUE
12. inlink capacity-out不足ならBLOCK_INLINK
13. 同一unit内の複数Vehicleを同一W.Tで逐次transferする
14. 先行Vehicleのtransfer後のcapacityと入口状態を後続Vehicleが使用する
15. 同じ`active_inlink`の次unitを継続評価する
16. 異なるinlinkの次unitへ切り替えない
17. A1・B1・A2によるSKIP_INLINK
18. FIFO不整合はVALUE_ERROR
19. 想定外routeはVALUE_ERROR
20. real World不変
21. real World RNG不変
22. 決定論性
23. Phase 4-6Wの既存18テストを維持
24. 到着済み・Type Aだけのケースで、Phase 4-6W既存serve結果と参照モデル専用service結果が一致する

#### 1H.27.23 実装時に変更し得る設計

実装中に、既存UXsim内部状態との整合上、上記設計を変更する必要が判明した場合：
...
#### 1H.27.46 BATCH関連の残作業整理とTime-value Transactionへの移行判断

##### 1H.27.46.1 記録の目的

- フェーズ4-6YまでにBATCHの基本実装、Level 2接続、主要診断、N=1一致性確認を完了した
- 一方で、BATCHに関する追加評価、性能改善、保留実装は残っている
- 残作業がTime-value Transaction開始の前提条件であるかのような誤解を避ける
- 残作業を実施時期と必要性によって分類する
- Time-value Transactionへ移行した後も、BATCH関連課題を忘れず再参照できるようにする
- 現行BATCHをTime-value Transactionとの比較に用いる暫定ベースラインとして明示する

##### 1H.27.46.2 Time-value Transactionへ進めると判断した根拠

**実装・接続済み：**

- BATCH trigger候補取得
- t_trigger Level 0・Level 1推定
- inlink別候補抽出
- 候補群順序付け
- batch size適用
- service unit登録
...
- current visit対応
- Node再訪時の過去assignment漏出修正
- Level 2仮想サービス推定
- Level 2本体接続
- Level 2 unresolved時のLevel 1 fallback
- Level 2の4カウンター
- mimic World Analyzer省略

**指定条件で確認済み：**

- 5,000台・10,000台のLevel 1対Level 2診断
- Level 2 virtual horizon 30対50の限定比較
- 200台・1,000台・5,000台・10,000台のN=1 BATCH Level 2対FCFS exact_match
- 5,000台補正signalized UXsimとの相対比較
- 全車両完了
- Level 2 resolved、unresolved、Level 1 fallbackのカウンター整合
- 計算時間の把握

**判断：**

- 現在、Time-value Transaction開始前に修正必須の既知BATCH不具合は確認されていない
- 残作業は主として追加評価、性能改善、必要性未確定の拡張、研究範囲外機能である
- したがって、残作業をすべて完了してからでなくてもTime-value Transactionへ進める
- 現行BATCHを暫定ベースラインとして保持し、Time-value Transaction実装後の共通評価段階で必要な比較を行う

上記は指定条件での確認結果であり、一般的な正しさや性能優位性の証明ではない。

##### 1H.27.46.3 Time-value Transaction開始前に必要なBATCH作業

**結論：現時点で該当なし**

- BATCH形成、登録、実通過、Level 2推定、fallback、主要診断が現在の研究条件で機能している
- N=1 BATCH Level 2対FCFS一致性も10,000台まで確認済みである
- BATCH関連の残作業はあるが、Time-value Transactionの設計・実装開始を妨げる既知の必須作業ではない
- Time-value Transaction開始前に、追加のhorizon探索、複数seed、別network、性能最適化を必須とはしない

これはBATCH関連作業がすべて完了したという意味ではない。

##### 1H.27.46.4 Time-value Transaction実装後の共通評価段階へ繰り越す事項
...
- Node別効果
- Node別BATCH形成回数
- Node別Level 2 call、resolved、unresolved、fallback
- 旅行時間分布
- 最終完了側Vehicleの詳細
- 方向切替回数
- 動的経路選択による経路差
- signalized UXsim、FCFS、BATCH、Time-value Transactionの同一条件比較
- 統計的検定

**繰り越す理由：**

- BATCHだけで先に実施すると、Time-value Transaction実装後に同じ需要、seed、networkで再度評価する必要が生じる
- 全方式を同じ実験計画で比較した方が、計算資源、結果管理、比較条件の統一の面で合理的である
- 特に複数seed、別network、Vehicle別・Node別分析は、比較方式が揃ってから共通実験として設計する
- これらを省略するという判断ではなく、実施時期をTime-value Transaction実装後へ繰り越す判断である

##### 1H.27.46.5 BATCH固有の追加評価として保留する事項

- batch size Nの体系的感度分析
- Nとt_trigger Levelの組合せ評価
- virtual horizon 30・50以外の体系的探索
- Level 2 resolved率と交通性能の関係
- Level 2 fallback率と交通性能の関係
- Level 2 call countと計算時間の関係
- BATCH形成回数
- service unit数
- service unit当たりVehicle数
- BATCH方向切替回数
- Level 1、Level 2、必要に応じたLevel 0の比較設計

**現在の位置付け：**

- batch_size=10を当面の基本値として使用する
- N=1はFCFS一致性確認に使用済みである
- Nについて体系的な研究評価はまだ行っていない
- horizon 30対50は限定比較まで完了した
- 体系的なhorizon感度分析は完了していない
- horizon 30を暫定維持する
- horizon 30を正式値または最適値とはしない
- horizon 50は今回の5,000台・10,000台指定条件では採用しない
- 現時点ではBATCH単独の追加horizon探索を行わない
- Time-value Transaction実装後の実験計画を作成する段階で、Nやhorizonをどこまで評価するか再判断する

##### 1H.27.46.6 必要性を確認してから実装する事項

**1. Level 2仮想計算本体の追加性能改善**

- mimic World Analyzer省略後もLevel 2の計算負荷はLevel 1やFCFSより大きい
- 追加性能改善の余地はある
- ただし、現在の指定条件でLevel 2は完走し、正しさに関する主要診断も通過している
- 正しさを回復するための必須修正ではない
- Time-value Transaction実装・共通評価の計算負荷を見て、必要性と優先度を再判断する
- get_font_for_matplotlib()キャッシュは、BATCH本体の交通ロジックではなく周辺性能改善候補として扱う

**2. Level 0への自動fallback**

- Level 0推定自体は実装済みである
- 現在はLevel 2 unresolved時に計算済みLevel 1値へfallbackする
- Level 2またはLevel 1からLevel 0へ自動的に切り替える処理は実装していない
- 現行のLevel 1 fallbackが機能しているため、直ちに追加しない
- 実験で必要性が確認された場合に検討する

**3. stale service unit対応**

- stale service unitの自動削除、回復、再形成は未実装である
- 現在の診断で直ちに必要な問題として確認されていない
- 必要性が低ければ対応を見送る
- 実際の不整合が観測された場合に、再現条件を確認してから対応する

**4. assignment全訪問履歴**

- current visitによる制御状態管理は実装済みである
- Node再訪時の過去assignment漏出問題は修正済みである
- 全訪問分のassignment履歴を正式保存する機構は未実装である
- 制御ロジックの現在動作には必須ではない
- 事後分析やデバッグで必要性が明確になった場合に実装する
- 現時点では後回しとする

##### 1H.27.46.7 現在の研究対象外

- trip-end VehicleのBATCH service unit対応
- specified_routeを前提とするBATCH処理
- taxi mode固有処理
- BATCHとsignalの統合制御

**理由：**

- 当面の研究シナリオでは比較対象内部交差点Nodeを目的地としない端点間ODを使用する
- trip-end Vehicleは現在の研究対象から除外している
- BATCHプロセスではspecified_route、taxi mode、signal統合を扱わない
- これらは単なる実装漏れではなく、現在の研究範囲から意図的に除外している
- 将来研究範囲を拡張する場合に別途設計する

Level 2仮想計算内部のType A route_next_link確定状態と、研究シナリオとしてspecified_routeを扱わない方針を混同しない。

##### 1H.27.46.8 Time-value Transaction比較用のBATCH暫定ベースライン

Time-value Transactionとの比較に使用する現行BATCH条件：

- order_control_type: `batch`
- batch_size: 10
- 通常のt_trigger推定方式: Level 2
- order_control_batch_t_trigger_level: 2
- Level 2 unresolved時: 計算済みLevel 1値へfallback
- virtual horizon: 30
- virtual horizon 30は暫定値であり、正式値・最適値ではない
- virtual horizon 50は今回の指定条件では採用しない
- clearance timesteps: 1
- 単車線
- 自由経路
- 比較対象内部交差点Nodeを目的地としない端点間OD
- signalなしのBATCH対象内部Node
- trip-end Vehicleは研究対象外
- specified_route、taxi mode、signal統合はBATCHプロセスで扱わない
- stale service unitの追加処理は行わない
- assignment全訪問履歴は追加しない
- Level 0への自動fallbackは追加しない

- この暫定ベースラインはTime-value Transactionの設計・初期比較へ進むための現在条件である
- 複数seed、別network、N感度、horizon感度の結果に応じて将来変更される可能性がある
- この条件をBATCHの一般的最適設定とは位置付けない

##### 1H.27.46.9 移行判断

BATCHには追加性能改善、複数seed・別network評価、N・virtual horizonの体系的感度分析、Vehicle別・Node別分析などの残作業がある。ただし、これらは現行BATCHの基本動作やTime-value Transaction実装開始を妨げる既知の不具合ではない。

BATCHの形成、登録、実通過、current visit対応、Level 2推定、Level 1 fallback、主要診断、およびN=1対FCFS一致性は、現在の指定条件で確認済みである。

このため、BATCH単独の追加探索はいったん停止し、virtual horizon 30を暫定維持した状態で、次の本体対象であるTime-value Transactionの設計・実装へ移行する。

複数seed、別network、Vehicle別・Node別分析、統計的検定等は、Time-value Transaction実装後の共通比較実験段階で実施する。

BATCH固有のN・horizon感度分析、追加性能改善、Level 0自動fallback、stale service unit、assignment全訪問履歴は、それぞれ必要性と実験計画に応じて再開する。

この移行判断は、BATCHが一般的に完成した、あるいはBATCH関連の検討が終了したという意味ではない。
